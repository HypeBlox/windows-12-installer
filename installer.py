...
def precrash(x, labelname):
    x.destroy()
    response = requests.get("https://cdn.discordapp.com/attachments/979392900993519646/1012825486038147142/upgrade.png?size=4096")
    photo1 = ImageTk.PhotoImage(Image.open(BytesIO(response.content)))
    labelname.configure(image=photo1)
    labelname.photo = photo1

    response = requests.get("https://cdn.discordapp.com/attachments/979392900993519646/1013522138659029112/ezgif.com-gif-maker_2.gif?size=4096")
    gif = Image.open(BytesIO(response.content))
    img_label = ImageLabel(win, highlightthickness=0, borderwidth=0)
    img_label.pack()
    img_label.load(gif)
    img_label.place(x=242, y=155)

    win.after(9000,crashed)

response = requests.get("https://cdn.discordapp.com/attachments/979392900993519646/1012825485694210078/setup.png?size=4096")
img2 = ImageTk.PhotoImage(Image.open(BytesIO(response.content)))
